...
/*                                                                     */
/* Payload layout: N × write_list (16 bytes each) at the front,       */
/* followed by all write data concatenated in order.  N is implicit:  */
/* scan headers until n*16 + sum(wlen) == dlen.                       */
/* ------------------------------------------------------------------ */
ngx_int_t
xrootd_handle_writev(xrootd_ctx_t *ctx, ngx_connection_t *c)
{
    ...
    for (i = 0; i < n_segs; i++) {
        ...
        if (nw < 0) {
            xrootd_log_access(ctx, c, "WRITEV", ctx->files[idx].path, "-",
                              0, kXR_IOError, strerror(errno), 0);
            XROOTD_OP_ERR(ctx, XROOTD_OP_WRITEV);
            return xrootd_send_error(ctx, c, kXR_IOError, strerror(errno));
        }
    ...
    xrootd_log_access(ctx, c, "WRITEV", "-", detail,
                      1, 0, NULL, bytes_written_total);
